...
Page({
  ...
  onReady: function () {
    ...
    util.req(url, 'GET', {}, function (data) {
      that.setData({
        topicPic: data.ads_question_publish
      });
    }, function () {
      util.req("请求接口失败");
    });
  ...
  formSubmit: function (e) {
    ...
    if (!that.data.uploadImageFlag) {
      console.log("上传图片的地址："+that.data.uploadImageUrl);
  ...
  onUploadUrl: function (data) {
    console.log("图片上传标志：" + data.detail.flag);
    this.setData({
  ...
  // 绑定输入
  bdinput:function (params) {
    this.setData({
      rzj: parseInt(params.detail.value)
    })
  }
})